...
# df_admissions <- read_csv("input/")
df_admissions <- read_csv("input/df_icu_adm_MV_2023-03-27_selected.csv")

df_covid_admissions_mv <- 
    df_admissions %>% 
    filter(lubridate::year(UnitAdmissionDate) %in% c(2022)) %>%
    ...
    mutate(
        IsRenalReplacementTherapy1h = case_when(
            AdmissionSourceName_group == "Emergência" & IsRenalReplacementTherapy1h == 1 ~ 0,
            TRUE ~ IsRenalReplacementTherapy1h
        )
    ) %>% 
    filter(!(IsRenalReplacementTherapy1h == 1)) %>% 
...
write_csv(df_covid_admissions_mv, "input/df_icu_adm_mv_filtrado_non_covid_2023-03-27_V2.csv")
